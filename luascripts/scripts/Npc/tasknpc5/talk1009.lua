...
function CreatureNpcFun1009(npc,event,player,funid)
  if funid == 2 and player:get_task_state(9) == 1 then
	  player:set_task_state(9,2); 
      player:set_task_state(10,1);  
      player:push_task_data(10,1);
      player:add_exp(15000);
      player:add_gamemoney_bind(8000);
      local job = player:get_job();
      if job == 1 then
    player:add_item(21001,1,1,0,0);
      elseif job == 2 then
    player:add_item(21022,1,1,0,0);
      else 
    player:add_item(21043,1,1,0,0);
      end     
	end  
end
