...
function CreatureNpcFun1008(npc,event,player,funid)
  if funid == 2 and player:get_task_state(8) == 1 then
      player:set_task_state(8,2); 
	  player:set_task_state(9,1);  
      player:push_task_data(9,1);
      player:add_exp(12000);
      player:add_gamemoney_bind(6000);
	local job = player:get_job();
	if job == 1 then
			player:add_item(20401,1,1,0,0);
		elseif job == 2 then
			player:add_item(20422,1,1,0,0);
		else
			player:add_item(20443,1,1,0,0);
		end
      return;
  end
end
