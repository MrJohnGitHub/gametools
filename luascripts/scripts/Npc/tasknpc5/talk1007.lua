...
function CreatureNpcFun1007(npc,event,player,funid)
  if funid == 2 and player:get_task_state(7) == 1 then
	  player:set_task_state(7,2);
      player:set_task_state(8,1);  
      player:push_task_data(8,1);
      player:add_exp(10000);
      player:add_gamemoney_bind(4500);
	    local job = player:get_job();
	    if job == 1 then
		player:add_item(20801,1,1,0,0);
	    elseif job == 2 then
		player:add_item(20822,1,1,0,0);
	    else 
		player:add_item(20843,1,1,0,0);
	    end      
      return;
  end
end
