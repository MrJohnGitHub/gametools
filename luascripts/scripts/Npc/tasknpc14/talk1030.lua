...
function CreatureNpcFun1030(npc,event,player,funid)
	if funid == 2 and player:get_task_state(30) == 1 then
		  player:set_task_state(30,2); 
		  player:set_task_state(31,1);  
		  player:push_task_data(31,1);
		  player:add_exp(500000);
		  player:add_gamemoney_bind(90000);
	    local job = player:get_job();
	    if job == 1 then
		player:add_item(20805,1,1,0,0);
	    elseif job == 2 then
		player:add_item(20826,1,1,0,0);
	    else 
		player:add_item(20847,1,1,0,0);
	    end		  
		  return;
	  end
end
