...
local ACT_ID = 17
local paramKey = 809
local TIME_PARAM_KEY = 818
function isBossScoreCompActivityRankOpen(player)
	-- body
	local conf, start, over = GetOperActivityConfig(g_BossScoreCompTimeConf)
	local open = 0
	if conf ~= nil then
		if player ~= nil then
...
function cleargatherBossScoreCompActivityRankInfo(player)
		player:set_param_action(paramKey, 0)
		player:set_param_misc(paramKey, 0)
		player:set_param_type(paramKey, 0)
end

function gatherBossScoreCompActivityRankInfo(player)
	...
	local award = g_BossScoreCompAwardConf[index] and g_BossScoreCompAwardConf[index].award
	if award ~= nil then
		retMsg.award = award
...
function getBossScoreCompActivityRealRank(rank, payNum, beforRank, index)
	if index  == 0 then
		local conf, start, over, confIndex = GetOperActivityConfig(g_BossScoreCompTimeConf)
		index = confIndex
	end
	local realRank = 0
	local rankConf = g_BossScoreCompAwardConf[index] and g_BossScoreCompAwardConf[index].award
...
function gatherBossScoreCompActivityRankStatus(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_BossScoreCompTimeConf)
	local open = 0
	local status = 0
	if conf ~= nil then
		open = 1
...
--活动结束
--通知C++发奖励
function BossScoreCompActivityOverNotifySendAward()
	local cur = os.time()
	local index = 0
	for k, v in pairs(g_BossScoreCompTimeConf) do
		local startTime = 0
		local endTime = 0
	...
	if index ~= 0 then
		...
		if award ~= nil then
			...
			if sendCount > 0 then
				g_MailSendBossScoreCompRankActivityAward(sendCount, index)
			end
		end
	end
end
...
--邮件发送消费排行榜活动奖励
function MailSendBossScoreCompAward(roleid, rank, confIndex)
	local rankConf = g_BossScoreCompAwardConf[confIndex] and g_BossScoreCompAwardConf[confIndex].award
	if rankConf and rankConf[rank] and rankConf[rank].awardList then
		...
		local subject = "BOSS积分排行奖励"
		local body = "恭喜你在BOSS积分排行活动中排行第<font color='#FFFF000'>"..rank.."</font>名，以下是给予您的奖励，请笑纳！"
...
function DuringActivityAddBossScoreCompScore(player, addCount)
	local notify = false
	local arr = {}
	local retMsg = 
	{
	...
	if isBossScoreCompActivityRankOpen(player) == 1 then
	...
	if CheckBossScoreIsOpen(player) == 1 then
		local value = player:get_param(807) + addCount
		player:set_param(807, value)
		--通知前端数据变化
		local info = {actid = 71, key = value}
		table.insert(arr, info)
		notify = true
	end
