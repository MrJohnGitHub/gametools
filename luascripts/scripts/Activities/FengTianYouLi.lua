...
local ACT_ID = 69
local InstanceParamKey = 802
local OPT_TYPE = 265
local CONSUME_TYPE = 117
local FLY = 0
local TIME_PARAM_KEY = 834

function clearFengTianYouLiActivityInfo(player)
...
function FengTianYouLiActivityInfo(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_FengTianYouLiConf)
...
function CheckFengTianYouLiCanGetAwardByIndex(player, conf, index)
...
function CheckFengTianYouLiCanGetAward(player, conf)
	for index , v in pairs(conf.award or {}) do
		local flag = CheckFengTianYouLiCanGetAwardByIndex(player, conf, index)
		if flag == true then
			return 1
		end
	end
...
function HandlerGetFengTianYouLiAwardByIndex(player, index)
	--判断
	local conf = GetOperActivityConfig(g_FengTianYouLiConf)
	if conf == nil then
	...
	local flag, errMsg = CheckFengTianYouLiCanGetAwardByIndex(player, conf, index)
	if flag == false then
		player:alert(10,0,0,errMsg)
		return 
	end
...
function IsFengTianYouLiActivityHasAward(player, conf)
	local flag = IsResetActivityAwardFlagInfo(player, conf, InstanceParamKey, TIME_PARAM_KEY, conf and conf.overtimeStamp)
	if flag == true then
		player:set_param_action(InstanceParamKey, 0)
		player:set_param_misc(InstanceParamKey, 0)
	...
	local status = CheckFengTianYouLiCanGetAward(player, conf)
	return status
end

function FengTianYouLiActivityStatus(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_FengTianYouLiConf)
	local open = 0
	if conf ~= nil then
		local status = IsFengTianYouLiActivityHasAward(player, conf)
		open = 1
...
function CheckFengTianYouLiIsOpen(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_FengTianYouLiConf)
	local open = 0
	if conf ~= nil then
		local flag = IsResetActivityAwardFlagInfo(player, conf, InstanceParamKey, TIME_PARAM_KEY, conf and conf.overtimeStamp)
...
function AddFengTianYouLiParamCount(player)
	if CheckFengTianYouLiIsOpen(player) == 1 then
		player:set_param(802,player:get_param(780))
		HandlerGatherOpenActivityInfo(player, ACT_ID)
	end
end