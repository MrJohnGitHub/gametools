...
local ACT_ID = 60
local InstanceParamKey = 767
local OPT_TYPE = 260
local CONSUME_TYPE = 260
local FLY = 0
local TIME_PARAM_KEY = 827

function clearXuanshenYouLiActivityInfo(player)
...
function XuanshenYouLiActivityInfo(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_XuanShenYouLiConf)
...
function CheckXuanShenYouLiCanGetAwardByIndex(player, conf, index)
...
function CheckXuanShenYouLiCanGetAward(player, conf)
	for index , v in pairs(conf.award or {}) do
		local flag = CheckXuanShenYouLiCanGetAwardByIndex(player, conf, index)
		if flag == true then
			return 1
		end
	end
...
function HandlerGetXuanShenYouLiAwardByIndex(player, index)
	--判断
	local conf = GetOperActivityConfig(g_XuanShenYouLiConf)
	if conf == nil then
	...
	local flag, errMsg = CheckXuanShenYouLiCanGetAwardByIndex(player, conf, index)
	if flag == false then
		player:alert(10,0,0,errMsg)
		return 
	end
...
function IsXuanShenYouLiActivityHasAward(player, conf)
	local flag = IsResetActivityAwardFlagInfo(player, conf, InstanceParamKey, TIME_PARAM_KEY, conf and conf.overtimeStamp)
	if flag == true then
		player:set_param_action(InstanceParamKey, 0)
		player:set_param_misc(InstanceParamKey, 0)
	...
	local status = CheckXuanShenYouLiCanGetAward(player, conf)
	return status
end

function XuanShenYouLiActivityStatus(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_XuanShenYouLiConf)
	local open = 0
	if conf ~= nil then
		local status = IsXuanShenYouLiActivityHasAward(player, conf)
		open = 1
...
function CheckXuanShenYouLiIsOpen(player)
	local conf, startTime, endTime = GetOperActivityConfig(g_XuanShenYouLiConf)
	local open = 0
	if conf ~= nil then
		local flag = IsResetActivityAwardFlagInfo(player, conf, InstanceParamKey, TIME_PARAM_KEY, conf and conf.overtimeStamp)
...
function AddXuanShenYouLiParamCount(player)
	if CheckXuanShenYouLiIsOpen(player) == 1 then
